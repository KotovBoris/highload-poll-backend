...
# --- 2b. Повторный GET всегда 200 (страница никогда не падает из-за лимита) ---
# Лимит выдачи (MAX_COOKIES_PER_CLIENT=1000 на IP) здесь намеренно НЕ исчерпан,
# поэтому cookie выдаётся снова — это ожидаемо. Проверяем главное: страница
# опроса отдаётся всегда, а не отдаёт ошибку. Исчерпание лимита покрыто
# интеграционными тестами (TestGetPoll_NeverFailsOnIssueLimit).
CODE=$(curl -s -o /dev/null -w '%{http_code}' "$API_URL/polls/$POLL_ID")
[ "$CODE" = "200" ] || fail "повторный GET: ожидался 200, получен $CODE"
log "Страница опроса отдаётся повторно (200) — лимит не ломает GET."
