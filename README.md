...
### Пиковая пропускная способность (Linux-стенд)

На macOS Docker Desktop потолок — ~12 тыс. RPS из-за виртуализации сети, поэтому
замеры проводились на Linux-стенде (22 ядра), стек в Docker, 2 API-воркера:

| Метрика | Значение |
|---|---|
| Пропускная способность | **~88 000 RPS** (2 клиента × 400) |
| Потери голосов | **0 (300 000 / 300 000)** |
| Latency p50 / p90 / p99 | ~0.75 мс / ~2 мс / ~5 мс |
| Узкое место | stdlib `net/http` — 76% CPU, наш код — 16% |

> **Один нагрузчик упирается в себя**: при `-c 600` он съедает ~8 ядер и показывает
> ~60K RPS — это потолок клиента, а не сервера. Ёмкость измеряется суммой
> нескольких клиентов.

Оптимизации по профилю дали **+50%** (58K → 88K): `json.Unmarshal` вместо
`Decoder`, один `Write` с предвычисленным ответом, `GOGC=400`.

Подробности, методика и разбор узких мест — в
