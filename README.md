...
**Время готовности управляется `CLOSE_GRACE`** — это пауза после `ends_at`, чтобы
хвостовые голоса и «done» успели дойти до consumer'а. Тот же сценарий с
`CLOSE_GRACE=1s`:

| `CLOSE_GRACE` | Готовность результатов | Потери |
|---|---|---|
| `5s` (в compose) | p50 ≈ 5.6 с, max ≈ 6.2 с | 0 |
| `1s` | **p50 ≈ 1.6 с, max ≈ 2.6 с** | 0 |

На локальной машине хватает 1 с, но в проде сетевые задержки выше, поэтому в
compose стоит 5 с с запасом. Значение — ручка баланса «быстрее результат» против
«гарантированно ничего не потерять», меняется через env без правки кода.
