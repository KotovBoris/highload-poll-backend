...
- **Анти-накрутка:** подпись cookie (выдуманный UUID не проходит) + fallback по
  ...
  Осознанно без DDoS-защиты; prod-путь (consistent hashing по `poll_id|IP` на
  балансере) — в [`04-architecture.md`](docs/architecture/04-architecture.md).
...
| Метрика | Значение |
|---|---|
| Пропускная способность | ~13–16 тыс. RPS |
| Ошибки | 0% |
| Latency p50 / p90 / p99 | ~10–20 ms / ~18–31 ms / ~30–78 ms |
| Распределение голосов | равномерное (38943/39240/39755/39063) |
| Дедупликация | подтверждена (уникальные голоса = учтённые) |

> Нагрузчик использует уникальные IP (`-unique-ips`), поэтому при fallback
> `sha256(IP)` каждый голос даёт свой fingerprint. Для сценария анти-накрутки
> есть отдельные интеграционные тесты.
